...
from lcquad2_dataloader import MyRLDataLoader
...
####载入数据####
mrldl = MyRLDataLoader(num_of_train_candidate_data=30)
...
cem_model.load_state_dict(torch.load("../datas/lcquad2_cem_model.pkl"))
    ...
    with torch.no_grad():
        ...
        for test_data in test_datas:
            ...
            if len(dic_relations) != 0:
                ...
                lcquad2_predictions.append(dic_relations[pred])
                lcquad2_golds.append(gold_relation)
                ...
                lcquad2_predictions.append(reduced_relations[pred])
                lcquad2_golds.append(gold_relation)

            lcquad2_p += (len(set(lcquad2_predictions) & set(lcquad2_golds))) / len(set(lcquad2_predictions))
            lcquad2_r += (len(set(lcquad2_predictions) & set(lcquad2_golds))) / len(set(lcquad2_golds))

    lcquad2_p /= len(test_datas)
    lcquad2_r /= len(test_datas)
    lcquad2_f1 = 2 * ((lcquad2_p * lcquad2_r) / (lcquad2_p + lcquad2_r))
    print(epoch, 'p: ', lcquad2_p, 'r: ', lcquad2_r, 'f1: ', lcquad2_f1)
