...
from qald9_dataloader import MyRLDataLoader
...
####载入数据####
mrldl = MyRLDataLoader(num_of_train_candidate_data=15)
...
cem_model.load_state_dict(torch.load("../datas/qald9_cem_model.pkl"))
    ...
    with torch.no_grad():
        ...
        for test_data in test_datas:
            ...
            if len(dic_relations) != 0:
                ...
                qald9_predictions.append(dic_relations[pred])
                qald9_golds.append(gold_relation)
                ...
                qald9_predictions.append(reduced_relations[pred])
                qald9_golds.append(gold_relation)

            qald9_p += (len(set(qald9_predictions) & set(qald9_golds))) / len(set(qald9_predictions))
            qald9_r += (len(set(qald9_predictions) & set(qald9_golds))) / len(set(qald9_golds))

    qald9_p /= len(test_datas)
    qald9_r /= len(test_datas)
    qald9_f1 = 2 * ((qald9_p * qald9_r) / (qald9_p + qald9_r))
    print(epoch, 'p: ', qald9_p, 'r: ', qald9_r, 'f1: ', qald9_f1)
